...
from pyspark.sql.functions import col, year, month, encode
...
spark = SparkSession.builder.appName("Monthly Consumption By Location Transformer").config(conf=conf).getOrCreate()
...
monthly_consumption_by_location = daily_consumption_df.join(location_meter_df, 'meter_id')\
    .groupBy(['location', 'longitude', 'latitude', 'year', 'month']).agg({'measure': 'sum'}).withColumnRenamed("sum(measure)", "consumption")
monthly_consumption_by_location.select(encode("location", "UTF-8"), 'longitude', 'latitude', 'consumption', 'year', 'month').show()

# Write the data back to Redshift
monthly_consumption_by_location.write.jdbc(url=redshift_url, table='serving.monthly_consumption_by_location' , mode='append', properties=redshift_properties)
