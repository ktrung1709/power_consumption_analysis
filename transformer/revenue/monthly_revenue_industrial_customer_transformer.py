...
spark = SparkSession.builder.appName("Monthly Revenue By Industrial Customer Transformer").config(conf=conf).getOrCreate()
...
query = "(select c.customer_id, c.customer_name, m.meter_id, m.voltage from cmis.customer c \
        ...
        where c.customer_type = 'industrial' ) as tmp"
industrial_customer_meter_df = spark.read.jdbc(redshift_url, query, properties=redshift_properties)


monthly_consumption_by_industrial_customer_df = industrial_customer_meter_df.join(daily_consumption_df, 'meter_id')\
...
cond = [industrial_price_df.voltage_tier == monthly_consumption_by_industrial_customer_df.voltage, \
        industrial_price_df.time_of_day == monthly_consumption_by_industrial_customer_df.time_of_day]
monthly_revenue_small_by_industrial_customer_df = monthly_consumption_by_industrial_customer_df.join(industrial_price_df, cond) \
    .withColumn("revenue", monthly_consumption_by_industrial_customer_df.consumption * industrial_price_df.price)
monthly_revenue_by_industrial_customer_df = monthly_revenue_small_by_industrial_customer_df.groupBy(['customer_id', 'customer_name', 'year', 'month'])\
    .agg({'revenue': 'sum'}).withColumnRenamed("sum(revenue)", "revenue")\
    .select('customer_id', 'customer_name', 'year', 'month', 'revenue')
monthly_revenue_by_industrial_customer_df.write.jdbc(url=redshift_url, table='serving.monthly_revenue_by_customer' , mode='append', properties=redshift_properties)
# Stop SparkSession
spark.stop()